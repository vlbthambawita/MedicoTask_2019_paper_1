...
# My own imports
# from Dataloaders.spermFeatureDataLoaderNormalized_unsqueezed_stackedFrames import SpermFeatureDatasetNormalizedUnsqueezed
from Dataloaders.new_spermDataloader_stackedDenseImages_withInTransforms import SpermDataset as spvdn
# from myNets.multi_input_net_v2 import Multiinput_net_v2
...
parser.add_argument("--data_roots", 
                    default=["/work/vajira/data/generated_250_outputs_stride_10_original_2nd_frame/generated_250_outputs_stride_10_original_2nd_frame",
                    "/work/vajira/data/dense_optical_flow_stride_1/generated_250_outputs",
                    "/work/vajira/data/dense_optical_flow_stride_10/generated_250_outputs_stride_10"],
...
parser.add_argument("--cols", default=["Head defects (%)", "Midpiece and neck defects (%)", "Tail defects (%)"])
...
parser.add_argument("--checkpoint_interval", type=int, default=10, help="Interval to save checkpoint models")
parser.add_argument("--fold", type=str, default="fold_3", help="Select the validation fold", choices=["fold_1", "fold_2", "fold_3"])
...
def prepare_data():
    ...
    dataset_all = {x: spvdn(opt.gt_csv_file,
                            opt.id_csv_file,
                            os.path.join(opt.data_roots[0], x),
                            os.path.join(opt.data_roots[1], x),
                            os.path.join(opt.data_roots[2], x),
                            opt.cols,
                            data_transforms
    ...
    # 2 folds to train 
    dataset_train = torch.utils.data.ConcatDataset([dataset_all[train_folds[0]], dataset_all[train_folds[1]]])
    
    # 1 fold to validation
    dataset_val = dataset_all[validation_fold]
...
def train_model(model, optimizer, criterion, criterion_validation, dataloaders: dict, scheduler):
    ...
    for epoch in tqdm(range(opt.start_epoch + 1, opt.start_epoch + opt.num_epochs + 1)):
        ...
        for phase in ["train", "val"]:
            ...
            for i, sample in tqdm(enumerate(dataloader, 0)):
                ...
                # handle input data
                input = sample["image_stack"]
                input = input.to(device, torch.float)

                # print("Input shape test=",input.shape)
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    outputs = model(input)
                    outputs_real = outputs #* 100 #  * std + mean

                    # Loss
                    loss = criterion(outputs, gt_real)
                    loss_real = criterion_validation(outputs_real, gt_real)
                ...
                running_loss += loss.detach().item() * input.size(0)
                running_loss_real+= loss_real.detach().item() * input.size(0)
...
def check_model_graph():
    ...
    summary(model, (9, 224, 224)) # this run on GPU
    model = model.to('cpu')
    #dataloaders = prepare_data()
   ...
   # inputs = inputs.to(device, torch.float)
    #print(inputs.shape)
    print(model)
    dummy_input = Variable(torch.rand(13, 9, 224, 224))
